...
const /** @type {?} */ TRANSLATION_PREFIX = 'MSG_';
/** @enum {number} */
...
export class ConstantPool {
    ...
    getTranslation(message, meta) {
        ...
        const /** @type {?} */ key = meta.meaning ? `${message}\u0000\u0000${meta.meaning}` : message;
        const /** @type {?} */ exp = this.translations.get(key);
        if (exp) {
            return exp;
        }
        const /** @type {?} */ docStmt = i18nMetaToDocStmt(meta);
        if (docStmt) {
            this.statements.push(docStmt);
        }
        // Call closure to get the translation
        const /** @type {?} */ variable = o.variable(this.freshTranslationName());
        const /** @type {?} */ fnCall = o.variable(GOOG_GET_MSG).callFn([o.literal(message)]);
...
class KeyVisitor {
    ...
    visitLiteralMapExpr(ast, context) {
        const /** @type {?} */ mapKey = (entry) => {
            const /** @type {?} */ quote = entry.quoted ? '"' : '';
            return `${quote}${entry.key}${quote}`;
        };
        const /** @type {?} */ mapEntry = (entry) => `${mapKey(entry)}:${entry.value.visitExpression(this, context)}`;
